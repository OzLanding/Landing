...
**중요**
`create()`나 `update()` 메서드는 오버라이드하지 마세요.
"객체가 실제 생성되는 기능"은 무조건 Service가 담당하고 책임져야 합니다.
Serializer는 "데이터가 유효한지" 검증하고 변환해 주는 일만 책임지게 해 주세요.
...
2. **Serializer는 검증만.** `create`/`update` 오버라이드 금지.
3. **Service가 모든 걸 한다.** 도메인 규칙, 권한, 상태 변경.
4. **Repository는 복잡한 쿼리가 있을 때만** 만든다.
5. **DB에 동시에 두 개 이상 써야 할 때는 `db_transaction.atomic()` 사용, 단일 작성은 감싸지 말 것.**
   ㄴ DB 작성(INSERT/UPDATE/DELETE) 여러 개가 하나의 동작으로 묶여야 할 때를 말함!
6. **예외는 `ValidationError` / `PermissionDenied`**로 Service에서 던진다.
7. **`django.db.transaction`은 항상 `as db_transaction`으로 별칭을 부여해 혼란 방지**.
